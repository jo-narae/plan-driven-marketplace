...
name: vercel-cicd
description: >-
  로컬 프로젝트를 GitHub 저장소로 올리고 Vercel에 연결해 CI/CD 자동 배포(main 푸시 시 프로덕션 자동 배포, PR/브랜치는 프리뷰 배포)를
  세팅한다. "vercel에 배포해줘", "깃허브에 올리고 배포", "ci/cd 세팅", "푸시하면 자동 배포되게", "라이브로 띄워줘", "프로덕션에
  올려줘" 같은 요청에 사용한다. 사용자가 "배포"만 말하고 GitHub/CI-CD를 명시하지 않아도, 로컬 웹 프로젝트를 실제로 띄우려는 맥락이면 이
  스킬을 쓴다. 전제: gh CLI 인증됨 + Vercel MCP 연결됨(+ Vercel CLI 로그인은 이 스킬 안에서 처리). 단순히 로컬에서
  npm run dev로 돌려보는 것만 원하면 이 스킬을 쓰지 않는다.
---
